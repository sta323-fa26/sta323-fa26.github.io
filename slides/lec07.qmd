...
::: callout-note
If the `gif` is not rendering, you might need some other dependencies, `install.packages(c("gifski","png"))`
:::
...
#| message: false
#| eval: false
amendments = read_csv("http://sta323-fa26.github.io/data/American_Amendments.csv")
```

Today's data set comes from [data.gov](https://catalog.data.gov/dataset/amending-america-proposed-amendments-to-the-united-states-constitution-1787-to-2014)
...
#   geom_density(aes(col = "> 60 minutes"))

# all_recipes %>%
#   mutate(moreThanHour = cook_time > 60) %>%
#   ggplot(aes(x = calories, color = moreThanHour))

all_recipes %>%
  filter(cook_time > (24*60)) %>%
  ggplot(aes(x = protein, y = avg_rating)) +
  geom_point() +
  labs("Average rating of meals that take longer than 24h to cook")

all_recipes %>%
  ggplot(aes(x = avg_rating, y = total_ratings)) +
  geom_point() +
  annotate(geom = "text", x = 1.5, y = 100, 
           label = "bad recipes", size = 5) +
  labs(title = "The best recipes get the most ratings")
```
...
#| eval: false
library(tidyverse)
x = read_csv("data/American_Amendments.csv")
x %>%
  mutate(century = case_when(year < 1800 ~ 18,
                             year < 1900 & year >= 1800 ~ 19,
                             year <2000 & year >= 1900 ~ 20, 
                             year >= 2000 ~ 21)) %>%
  ggplot(aes(x = century)) +
  geom_bar() +
  labs(title = "Amendments by century") +
  theme_bw()
```
