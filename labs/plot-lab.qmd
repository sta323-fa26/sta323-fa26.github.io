...
Using the untidy data set `us_rent_income`, from the package `tidyr` to re-create the plot below. Pay attention to re-creating exactly all aspects of the plot, e.g. axes, labels, capitalization etc.

![](images/lab-3-plot.png){width="800"}
...
-   The color used is `steelblue`.

-   You will need to find an external data set that you can merge to group states by region.

-   Double check that your plot renders nicely. You might need to adjust `fig-width` and `fig-height` YAML options [read here](https://quarto.org/docs/computations/execution-options.html). The ratio is 6:4 (width:height)
:::
