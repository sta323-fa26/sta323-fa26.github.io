---
title: "Lab 3: wrangling the written word"
subtitle: "Due Monday September 21 at 5:00pm"  
editor: source
execute:
  eval: false
---
...
-   In the *terminal* tab, type `cd ~` to navigate to your home directory. Next `git clone git@github.com:sta323-fa26/lab-3-username.git` where `username` is replaced with your username.
...
Why are regular expressions preferred to examining newsgroup by line number?
...
Now using all 4911 documents together with your functions from exercise 1 and 2, construct a data frame called `emails_df` that contains the following 4 columns: 

- `id`: the actual file name that uniquely identifies the email, e.g. `37916`.
- `newsgroup1`: the first newsgroup class (e.g. `sci.space`)
- `newsgroup2`: the second newsgroup class (if available) otherwise, NA
...
1. Are any of the "sender" in your data frame NA? If so, examine one of those emails and explain why.
2. What total number of emails have only 1 newsgroup label (of the 5 categories above)?
3. What proportion of emails originate from an academic "edu" sender address?
4. What proportion of emails in each `newsgroup1`, originate from a Duke email address?
...
We will comb through 4911 documents and construct a single data frame from these documents for use in a future lab. Our goal is to create a data frame that looks like the following:
